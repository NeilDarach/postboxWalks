...
function setPosition(position,dot,track,map)
{
  ...
  last = points.Kb[points.length-1];
...
function addMarkers(url,visited,notVisited,map) {
  getJSON(url,function(err, data) {
  if (err == null) {
    data.forEach((elem) => {
      ...
      const marker = new google.maps.Marker({
                        ...
                        icon: (elem.visited?visited:notVisited) });
...
function initMap() {
  ...
  addWalks("routes-old.sh",walkProperties,map);
  addMarkers("postboxes.json", svgMarkerVisited, svgMarker, map);
